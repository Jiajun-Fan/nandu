...
void AuthServerService::handleAuthInit(Service* service, Session& session, const Operation& in, Operation& out) {
    AuthServerService* svc = dynamic_cast<AuthServerService*>(service);
    assert(svc);

    std::string token = svc->generateToken();
    Operation send(kOpInit);
    send.fromString(token);
    session.data = token;
    session.curState = kScWaitHash;
    send.write(session.fd);
};

void AuthServerService::handleAuthHash(Service* service, Session& session, const Operation& in, Operation& out) {
    AuthServerService* svc = dynamic_cast<AuthServerService*>(service);
    assert(svc);
    ...
    if (expectedHash == hashStr) {
        ...
        Operation send(OP_DONE);
        send.fromString(std::string("Bad hash string"));
        send.write(session.fd);
...
void AuthClientService::handleAuthInit(Service* service, Session& session, const Operation& in, Operation& out) {
    ...
    Operation send(kOpHash);
    send.fromString(hashStr);
    send.write(session.fd);

    session.curState = kScWaitOK;
}

void AuthClientService::handleAuthOK(Service* service, Session& session, const Operation& in, Operation& out) {
    session.curState = SC_INIT;
    Info("C: Auth OK.\n");
}
